...
Compiles `TranslationEntry` objects to Qt's .ts XML format for a specific language.
...
Groups translations by context (widget class name or `@default`).
...
Includes disambiguation comments for same source text with different meanings.
...
Includes translator notes as extra comments in the .ts file.
...
Handles plural translations with numerus forms.
...
Extracts all unique language codes from translation entries.
...
Writes .ts files to disk, one per language.
...
Compiles only specified languages instead of all available languages.
