...
def _wrap_init_for_window(cls: type[Window[Any]]) -> None:
    ...
    def wrapped_init(self: Window[Any], *args: Any, **kwargs: Any) -> None:
        ...
        # Apply title inheritance (if no explicit title set)
        # - None in widget_props: inherit from QApplication's qtpie_window_title property
        # - explicit value: already applied by apply_widget_props above
        if "windowTitle" not in config.widget_props:
            from qtpy.QtWidgets import QApplication

            qapp = QApplication.instance()
            if qapp is not None:
                app_title = qapp.property("qtpie_window_title")
                if app_title:
            ...
            if qt_layout is not None:
                ...
                for name in getattr(cls, "__annotations__", {}):
                    ...
                    if annotation is HorizontalLine and name not in config.fields:
                        line = _create_horizontal_line(central)
                    ...
                    if annotation is VerticalLine and name not in config.fields:
                        line = _create_vertical_line(central)
                        setattr(self, name, line)
                        _add_to_layout(qt_layout, line, config.layout, None, None, None)
                        continue
                    ...
                    if annotation is Line and name not in config.fields:
                        line = _create_line_for_layout(central, qt_layout)
                        setattr(self, name, line)
                        _add_to_layout(qt_layout, line, config.layout, None, None, None)
                        continue
                    ...
                    if name in config.fields:
                        ...
                        if field.is_line:
                            line = _create_line_for_layout(central, target)
                            setattr(self, name, line)
                            _add_to_layout(target, line, config.layout, None, None, None)
                            continue
...
def _create_dock_fields(window: Window[Any], config: WindowConfig) -> None:
    ...
    for name in ordered_names:
        ...
        # Apply initial size (width=/height=) via resizeDocks() after window is shown
        # resize() doesn't work on dock widgets - Qt's dock system controls sizing.
        # We defer to after show() using a single-shot timer.
        ...
        if fld.initial_width is not None or fld.initial_height is not None:
            ...
            init_w = fld.initial_width
            init_h = fld.initial_height
...
def _setup_dock_visible_binding(window: Window[Any], dock: Any, binding: str) -> None:
    ...
    if is_format_string(binding):
        ...
        create_expression_binding(window, binding, set_dock_visible)
        return
