    ...
    def _setup_new_dock_hooks(self) -> None:
        ...
        for dock in self._window.findChildren(QDockWidget):
            ...
            def make_handler(d: QDockWidget) -> Callable[..., None]:
                return lambda *_args: _update_title_bar_for_dock(self._window, d, self._config, self._dock_overrides)
...
def _setup_title_bar_hooks(
    ...
    for dock in window.findChildren(QDockWidget):
        ...
        def make_handler(d: QDockWidget) -> Callable[..., None]:
            return lambda *_args: _update_title_bar_for_dock(window, d, config, dock_overrides)
...
def _update_title_bar_for_dock(
    ...
    if is_tabified and not dock.isFloating():
        _hide_titlebar(dock)
    else:
        _show_titlebar(dock)
